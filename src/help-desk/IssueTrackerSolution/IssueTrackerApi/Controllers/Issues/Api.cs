...
public class Api(IDocumentSession session) : ControllerBase
{
    ...
    [HttpPost("/issues-rpc")]
    public async Task<ActionResult> AddIssueWithRpcAsync([FromBody] CreateIssueRequestModel request,
        ...
        [FromServices] SupportHttpClient client)
    {
        ...
        if (results.IsValid)
        {
            ...
            var supportInfo = await client.GetCurrentSupportInformatiaonAsync();
